...
export function DeliveryAddressCard({ 
  ...
  return (
    <div 
      className={`bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100 relative overflow-hidden transition-all duration-300 ${
    ...
    >
      ...
      {isComplete && !isDisabled && (
        <div className="absolute -top-2 -right-2 z-20">
          <div className="relative">
            ...
            <div className="relative bg-[#3CB371] rounded-full p-1.5 shadow-lg">
      ...
      {/* Primary Badge */}
      {isPrimary && (
        <div className="absolute top-3 right-3 md:top-4 md:right-4 px-2.5 py-1 md:px-3 md:py-1.5 bg-black rounded-full flex items-center gap-1.5 text-[10px] md:text-xs font-bold text-white shadow-md">
          <IoCheckmarkCircle className="w-3 h-3 md:w-3.5 md:h-3.5" />
          <span>PRIMARY</span>
        </div>
      )}
      ...
      {isDisabled && (
        <div className="absolute top-3 right-3 md:top-4 md:right-4 px-2.5 py-1 md:px-3 md:py-1.5 bg-gray-400 rounded-full text-[10px] md:text-xs font-bold text-white shadow-md">
      ...
      {/* Header */}
      <div className="flex items-center gap-3 mb-5 md:mb-6 pr-24 md:pr-32">
        <div className={`w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center shadow-md transition-all ${
          isComplete 
            ? 'bg-[#3CB371] scale-105' 
            : 'bg-gradient-to-br from-gray-100 to-gray-200'
        }`}>
          <IoLocation className={`w-6 h-6 md:w-7 md:h-7 ${isComplete ? 'text-white' : 'text-gray-600'}`} />
        </div>
      ...
      {/* Action Buttons */}
      <div className="flex flex-col xs:flex-row gap-2 md:gap-3 mb-5 md:mb-6">
        {/* Geolocation Button */}
        <button
          onClick={async () => {
          ...
          className="w-full h-10 md:h-11 px-4 rounded-xl flex items-center justify-center gap-2 text-xs md:text-sm font-semibold transition-all bg-gray-100 text-gray-700 hover:bg-gray-200 border border-gray-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isGeolocationLoading ? (
            <Spinner className="w-4 h-4 text-gray-700" />
          ) : (
            <IoNavigate className="w-4 h-4 md:w-4.5 md:h-4.5" />
          )}
          ...
          <div className="relative flex-1">
            ...
            >
              {isCopyingAddress ? (
                ...
                <IoCopy className="w-4 h-4 md:w-4.5 md:h-4.5" />
              )}
              <span>Copy to...</span>
            </button>
      ...
      <div className="space-y-4 md:space-y-5">
        {/* Street Address */}
        <div>
          <label className="block text-xs md:text-sm font-bold mb-2 text-black">
          ...
          <div className="relative">
            <input
              ...
              required
              disabled={isDisabled}
              className={`w-full h-11 md:h-12 px-4 pr-10 md:pr-12 rounded-xl text-sm md:text-base text-black bg-white placeholder:text-gray-400 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                streetError 
            ...
            {address?.street && (
              <div className="absolute right-3 md:right-4 top-1/2 -translate-y-1/2">
                {isStreetValid(address.street) ? (
                  <IoCheckmarkCircle className="w-5 h-5 text-[#3CB371]" strokeWidth={2.5} />
                ) : (
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 md:gap-4">
          ...
          <div>
            ...
            <input
              ...
              value="Coimbatore"
              disabled
              className="w-full h-11 md:h-12 px-4 rounded-xl text-sm md:text-base border-2 border-gray-200 text-gray-500 bg-gray-50 cursor-not-allowed"
            />
            <p className="text-[10px] md:text-xs mt-1.5 text-gray-400">
