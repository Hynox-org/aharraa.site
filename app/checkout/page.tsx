...
export default function CheckoutPage() {
  ...
  }, [JSON.stringify(deliveryAddresses), setSelectedPlan, setSelectedDates, setSelectedVendorId]) // Re-run effect if deliveryAddresses or store setters change
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-green-100">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-neutral-900">{selectedPlan.name}</p>
              </div>
            </div>
            <div className="flex items-baseline gap-2 pt-3 border-t border-green-100">
              <span className="text-3xl font-bold text-green-600">₹{selectedPlan.totalPrice}</span>
            ...
            <div className="bg-gradient-to-r from-green-500 to-emerald-500 rounded-2xl p-5 text-white shadow-md">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-base font-bold">{selectedVendor.name}</p>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="flex items-center gap-3">
                <div className="flex-shrink-0 w-16 h-16 bg-gradient-to-br from-green-100 to-emerald-100 rounded-xl flex flex-col items-center justify-center">
                  <span className="text-2xl font-bold text-green-700">
                    {format(selectedDates.startDate, "dd")}
                  </span>
                  <span className="text-xs text-green-600 font-medium">
                    {format(selectedDates.startDate, "MMM")}
                  </span>
                </div>
                <div className="flex-1">
                  <p className="text-xs text-neutral-500 mb-1">Start Date</p>
                  <p className="text-sm font-semibold text-neutral-900">
                    {format(selectedDates.startDate, "EEEE, PPP")}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex-shrink-0 w-16 h-16 bg-gradient-to-br from-emerald-100 to-teal-100 rounded-xl flex flex-col items-center justify-center">
                  <span className="text-2xl font-bold text-emerald-700">
                    {format(selectedDates.endDate, "dd")}
                  </span>
                  <span className="text-xs text-emerald-600 font-medium">
                    {format(selectedDates.endDate, "MMM")}
                  </span>
                </div>
                <div className="flex-1">
                  <p className="text-xs text-neutral-500 mb-1">End Date</p>
                  <p className="text-sm font-semibold text-neutral-900">
                    {format(selectedDates.endDate, "EEEE, PPP")}
                  </p>
                </div>
              </div>
            </div>
